...
export function getTemperatures(
  temperatureType: TemperatureStyle
): Promise<Temperature[]> {
...
function getAllTemperatures(): Promise<Temperature[]> {
  return Promise.all([getDevices(), getTemperatureSensors()]).then(
    ([hueDevices, hueTemperatures]) =>
      toTemperatures(hueDevices, hueTemperatures)
  );
}
...
function toTemperatures(
  hueDevices: HueDevice[],
  hueTemperatures: HueTemperatureSensor[]
) {
  return hueTemperatures.map((hueTemperature) =>
    toTemperature(hueDevices, hueTemperature)
  );
}

function toTemperature(
  hueDevices: HueDevice[],
  hueTemperature: HueTemperatureSensor
): Temperature {
  const hueDevice = hueDevices.find(
    (hueDevice) => hueDevice.id === hueTemperature.owner.rid
  );
  ...
  return hueDevice
    ? {
        ...
        temperature: roundTemperatureToOneDecimal(
          hueTemperature.temperature.temperature
        ),
      }
    : {
